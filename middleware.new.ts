...
export default withAuth(
  function middleware(req) {
    ...
    // If token is missing, let NextAuth handle the redirect instead of forcing one here
    if (!token) {
      return NextResponse.next();
    }
  ...
  {
    callbacks: {
      // Only run our logic on protected areas; otherwise allow through
      authorized: ({ token, req }) =>
        publicPaths.some(p => req.nextUrl.pathname.startsWith(p)) || !!token
    }
  }
)
